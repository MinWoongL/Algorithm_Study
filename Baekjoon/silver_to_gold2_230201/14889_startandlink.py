...
comb_li = comb(comb_li, N//2)  # 팀이 나눠질수 있는 모든 조합을 구함
# print(comb_li)
idx = 0
ans = 10000000
while idx <= len(comb_li)//2:
    for i in range(len(comb_li[idx])):
        for j in range(i+1, len(comb_li[idx])):
            team_a += n_mat[comb_li[idx][i]-1][comb_li[idx][j]-1]  # 조합의 첫번째와 마지막은 서로 다른 대칭되는 인덱스
            team_a += n_mat[comb_li[idx][j]-1][comb_li[idx][i]-1]
    for i in range(len(comb_li[idx])):
        for j in range(i+1, len(comb_li[idx])):
            team_b += n_mat[comb_li[-idx-1][i]-1][comb_li[-idx-1][j]-1]  # 가장 앞, 가장 뒤 를 한칸씩 당기면서 순서대로 비교
