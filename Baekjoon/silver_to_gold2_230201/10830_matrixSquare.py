    ...
    for k in range(length):
        for l in range(length):
            for i in range(length):
                ...
                ans_mat[k][l] = ans_mat[k][l] % 1000  # 나머지 연산을 안에서 해줘야 시간초과가 나지않는다.
...
# print(*[*list(map(lambda x: x%1000, [value for value in v]) for v in ans)], sep='\n')
# print(*[map(lambda x: x%1000, [i for i in ans])])
# print(*list(map(lambda x: x%1000, [idx for idx in arr])))
